        ...
        try:
            ...
            if 0 <= choice < len(available_lifelines):
                ...
                lifeline_actions = {
                    ...
                    "Audience Poll": lambda: (
                        self._use_audience_poll(question),
                        question["options"],
                    )[1],
                    "Phone a Friend": lambda: (
                        self._use_phone_friend(question),
                        question["options"],
                    )[1],
                    "Ask the Expert": lambda: (
                        self._use_ask_expert(question),
                        question["options"],
                    )[1],
                }
    ...
    def ask_question(self, question: Dict, question_num: int) -> Optional[bool]:
        ...
        while True:
            ...
            if answer == "LIFELINE" and available_lifelines:
                current_options = self._handle_lifeline(
                    question, lifelines_copy, available_lifelines
                )
                ...
                print(
                    "❌ Invalid input. Please enter A, B, C, D, 'lifeline', or 'quit'"
                )
                time.sleep(1)
        ...
        for question in selected_questions:
            ...
            if result is None:  # Quit
                ...
                if self.score == len(selected_questions):
                    ...
                    print("\n🎊🎊 CONGRATULATIONS! 🎊🎊")
                    print(
                        f"🏆 You are the CROREPATI! You won Rs. {self.prizes[self.score-1]:,}!"
                    )
                    break
                else:
